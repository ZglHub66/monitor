  ...
  var MethodLibrary = (function () {
    return {
      ...
      getCanvasFingerprint: function () {
        ...
        var bin = atob(b64);
        return (MethodLibrary.bin2hex(bin.slice(-16, -12)));
      ...
      createUUID: function createUUID() {
        const fp = MethodLibrary.getCanvasFingerprint();
        return fp;
      },
      // 获取匹配库
      ...
      // 获取操作系统版本
      getOSVersion: function (options) {
        var _this = this
        var u = VariableLibrary.navigator.userAgent || {}
        var _windowsVersion = null
        if (options._windowsVersion && options._windowsVersion != '') {
          _windowsVersion = options._windowsVersion;
        }
        // win11异步暂不处理
        // if (typeof VariableLibrary.navigator.userAgentData != 'undefined') {
        //   VariableLibrary.navigator.userAgentData.getHighEntropyValues(["platformVersion"]).then(function (ua) {
        //     if (VariableLibrary.navigator.userAgentData.platform === "Windows") {
        ...
        _this.isWebview = MethodLibrary.getMatchMap['isWebview'];
  ...
  var LogicLibrary = (function () {
    return {
      DeviceInfoObj: function (params) {
        ...
        let _windowsVersion = { _windowsVersion: params._windowsVersion ? params._windowsVersion : '' };
        var info = {
          deviceType: MethodLibrary.getDeviceType(), // 设备类型
          OS: MethodLibrary.getOS(), // 操作系统
          OSVersion: MethodLibrary.getOSVersion(_windowsVersion), // 操作系统版本
          ...
          userAgent: VariableLibrary.navigator.userAgent, //浏览器指纹 包含 appCodeName,appName,appVersion,language,platform 等
          UUID: MethodLibrary.createUUID(),//生成通用唯一标识
          canvas: !!window.CanvasRenderingContext2D,// 是否支持canvas
          webgl: MethodLibrary.webgl()// 是否支持webgl
        }
...
// export default DeviceInfo; // 需要作为功能模块时添加 